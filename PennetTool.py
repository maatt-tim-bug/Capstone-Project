...
def create_report():
    ...
    subnets_input, host_up, ip_scan_service, nmap_result, wifi_essid, wifi_channel, wifi_bssid, wifi_encrypt, wifi_password, wifi_version, router,router_ip,router_version, router_vulnerability,router_username ,router_usrpw,router_cvss = update_data_from_file()
    ...
    strings_to_write.extend([
        ...
        f"   \n*For this step, we'll process all the useful information collected after scanning and narrow down which devices are vulnerable. Then, we're going to plan a scenario for an attack to verify it.\n",
        f" 1. Wi-fi Network: {wifi_essid}",
        f"  - Vulnerabilities:",
        f"   + Weak encryption algorithms",
        f"(https://knowledge-base.secureflag.com/vulnerabilities/broken_cryptography/weak_cipher_vulnerability.html)",
        f"   + Default or easily guessable passwords",
        f"(https://owasp.org/www-project-top-10-insider-threats/docs/2023/INT07_2023-Insecure_Passwords_and_Default_Credentials)",
        f"   + Lack of intrusion detection mechanisms",
        f"(https://owasp.org/www-community/controls/Intrusion_Detection)\n",
        f" 2. Router: {router}", 
        f"  - Vulnerabilities:",
        f"   + Open ports (e.g., telnet, http) susceptible to exploitation",
        f"(https://www.all-about-security.de/identifying-secure-and-unsecured-ports-and-how-to-secure-them/)",
        f"   + Default or weak credentials for administrative access",
        f"(https://www.acunetix.com/vulnerabilities/web/web-application-default-weak-credentials/)",
        f"   + Lack of firmware update or security patches",
        f"(https://support.uidaho.edu/TDClient/40/Portal/KB/ArticleDet?ID=2770)\n\n",
        f"III/ EXPLOIT\n",
        f" 1. Access Point: {wifi_essid}",
        f"  *Practically, we can employ various methods for exploitation, but they may consume a significant amount of time. Therefore, in this case, we are opting for a Dictionary Attack to simulate the scenario.",
        f"  - ESSID: {wifi_essid}",
        f"  - BSSID: {wifi_bssid}",
        f"  - Channel: {wifi_channel}",
        f"  - Encrypt: {wifi_encrypt}",
        f"  - Password: *{wifi_password}*{' => Strong' if is_strong_password else ' => Weak'}\n",
        f" 2. Cisco Router: {router}",
        f"  a. FTP",
        f"  b. SSH",
        f"  c. Telnet",
        f"   *After analysis, we're realize that port 23(telnet) and port 80(http) are opening. Let's check it out",
        f"   - Attack method: {router_vulnerability}",
        f"    + Username : {router_username}",
        f"    + Password : {router_usrpw}",
        f"    => Using this username, password for login in *Web Administration Panel*\n\n\n\n\n\n\n",
        f"IV/ RECOMMENDATION\n",
        f"  *Based on our analysis and findings, we recommend the following actions:",
        f"  1. Strengthen Wi-Fi Network Security:",
        f"     - Upgrade encryption algorithms from WEP or WPA to the latest standard, WPA3, to ensure stronger protection against unauthorized access.",
    ...
    for string_to_write in strings_to_write:
        if string_to_write.startswith("I/ RECONNAISSANCE"):
        ...
        elif string_to_write.startswith(" 1. Wi-fi Network:"):
            write_with_formatting(pdf, string_to_write, bold=True)
            ...
            write_with_formatting(pdf, string_to_write, bold=True)
        elif string_to_write.startswith(" 2. Cisco Router:"):
            write_with_formatting(pdf, string_to_write, bold=True)
        elif string_to_write.startswith("IV/ RECOMMENDATION"):
            write_with_formatting(pdf, string_to_write, bold=True, italic=True)
...
def update_data_from_file():
    try:
        ...
        from data import subnets_input, host_up, ip_scan_service, nmap_result, wifi_essid, wifi_channel, wifi_bssid, wifi_encrypt, wifi_password, wifi_version, router, router_ip, router_version, router_vulnerability,router_username ,router_usrpw ,router_cvss
    except ImportError:
        print("Error: Unable to import data from data.py")
        return None
    
    return subnets_input, host_up, ip_scan_service, nmap_result, wifi_essid, wifi_channel, wifi_bssid, wifi_encrypt, wifi_password, wifi_version, router,router_ip , router_version, router_vulnerability,router_username ,router_usrpw ,router_cvss
